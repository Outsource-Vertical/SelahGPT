...
export default function EditFaithProfileScreen({ route, navigation }) {
  ...
  const [theologicalStyle, setTheologicalStyle] = useState(
    profile.theologicalStyle || "",
  );
  const [prayerSchedule, setPrayerSchedule] = useState(
    profile.prayerSchedule || "",
  );
  const [spiritualGoals, setSpiritualGoals] = useState(
    profile.spiritualGoals?.join(", ") || "",
  );
  const [favoriteScriptures, setFavoriteScriptures] = useState(
    profile.favoriteScriptures?.join(", ") || "",
  );
  const [accountabilityPartner, setAccountabilityPartner] = useState(
    profile.accountabilityPartner || "",
  );
  const [devotionPreference, setDevotionPreference] = useState(
    profile.devotionPreference || "verse of the day",
  );
  const [currentStruggles, setCurrentStruggles] = useState(
    profile.currentStruggles?.join(", ") || "",
  );
  const [wantsScriptureEncouragement, setWantsScriptureEncouragement] =
    useState(profile.wantsScriptureEncouragement || false);
  ...
  const handleSave = async () => {
    await saveFaithProfile(user.uid, {
      ...
      currentStruggles: currentStruggles
        ? currentStruggles.split(",").map((str) => str.trim())
        : [],
  ...
  return (
    <ScrollView style={{ padding: 16 }}>
      ...
      <TextInput
        ...
        onChangeText={setSpiritualBackground}
        style={inputStyle}
      />

      <Text>Theological Style</Text>
      <TextInput
        value={theologicalStyle}
        onChangeText={setTheologicalStyle}
        style={inputStyle}
      />
      ...
      <TextInput
        ...
        onChangeText={setPrayerSchedule}
        style={inputStyle}
      />
      ...
      <TextInput
        ...
        onChangeText={setSpiritualGoals}
        style={inputStyle}
      />

      <Text>Favorite Scriptures (comma-separated)</Text>
      <TextInput
        value={favoriteScriptures}
        onChangeText={setFavoriteScriptures}
        style={inputStyle}
      />
      ...
      <TextInput
        ...
        onChangeText={setAccountabilityPartner}
        style={inputStyle}
      />

      <Text>Devotion Preference</Text>
      <TextInput
        value={devotionPreference}
        onChangeText={setDevotionPreference}
        style={inputStyle}
      />
      ...
      >
        ...
        <Switch
          value={wantsScriptureEncouragement}
          onValueChange={setWantsScriptureEncouragement}
        />
      </View>
