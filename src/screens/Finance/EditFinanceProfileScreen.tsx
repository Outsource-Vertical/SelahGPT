...
export default function EditFinanceProfileScreen({ route, navigation }) {
  ...
  const [budgetStyle, setBudgetStyle] = useState(
    profile.budgetStyle || "zero-based",
  );
  const [usesBudgetApp, setUsesBudgetApp] = useState(
    profile.usesBudgetApp || false,
  );
  ...
  const [spiritualViewOnMoney, setSpiritualViewOnMoney] = useState(
    profile.spiritualViewOnMoney || "",
  );
  const [wantsScriptureFinancialAdvice, setWantsScriptureFinancialAdvice] =
    useState(profile.wantsScriptureFinancialAdvice || false);
  const [financialGoals, setFinancialGoals] = useState(
    profile.financialGoals?.join(", ") || "",
  );
  const [financialStressors, setFinancialStressors] = useState(
    profile.financialStressors?.join(", ") || "",
  );
  ...
  const handleSave = async () => {
    await saveFinanceProfile(user.uid, {
      ...
      financialStressors: financialStressors
        .split(",")
        .map((str) => str.trim()),
    });
    navigation.goBack();
  ...
  return (
    <ScrollView style={{ padding: 16 }}>
      ...
      <TextInput
        value={moneyMindset}
        onChangeText={setMoneyMindset}
        style={inputStyle}
      />

      <Text>Spiritual View on Money</Text>
      <TextInput
        value={spiritualViewOnMoney}
        onChangeText={setSpiritualViewOnMoney}
        style={inputStyle}
      />
      ...
      <TextInput
        value={financialStressors}
        onChangeText={setFinancialStressors}
        style={inputStyle}
      />
      ...
      >
        ...
        <Switch
          value={wantsScriptureFinancialAdvice}
          onValueChange={setWantsScriptureFinancialAdvice}
        />
      </View>
